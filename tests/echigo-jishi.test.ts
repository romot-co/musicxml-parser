...
const filePath = path.resolve(
  __dirname,
  "../reference/xmlsamples/Echigo-Jishi.musicxml",
);
...
describe("Echigo-Jishi.musicxml Parser Test", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error(
        `Failed to read or parse Echigo-Jishi.musicxml: ${(e as Error).message}`,
      );
      scorePartwise = null;
    }
  });
  ...
  it("should parse defaults fonts and language", () => {
    ...
    expect(scorePartwise.defaults?.wordFont?.fontFamily).toBe(
      "Times New Roman",
    );
  ...
  it("should parse a specific measure and its contents (Measure 1)", () => {
    ...
    const allegroDirection = directionsArray.find(
      (d) => d.direction_type?.[0]?.words?.text === "Allegro",
    );
    expect(allegroDirection).toBeDefined();
    expect(allegroDirection?.placement).toBe("above");
    expect(
      allegroDirection?.direction_type?.[0].words?.formatting?.fontWeight,
    ).toBe("bold"); // Assuming words has formatting in your schema
