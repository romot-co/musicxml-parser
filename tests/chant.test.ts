...
import type { ScorePartwise, Measure, Note, Attributes, Direction, Lyric } from '../src/types';
import { NoteSchema, AttributesSchema, DirectionSchema, LyricSchema } from '../src/schemas';

const filePath = path.resolve(__dirname, '../reference/xmlsamples/Chant.musicxml');
...
describe('Chant.musicxml Parser Test', () => {
  ...
  try {
    ...
    console.error(`Failed to read or parse Chant.musicxml: ${(e as Error).message}`);
    scorePartwise = null;
  }
  ...
  it('should parse defaults including fonts and lyric language', () => {
    ...
    expect(scorePartwise.defaults?.lyricFonts?.[0].fontFamily).toBe('Book Antiqua');
  ...
  it('should parse Measure 1 attributes, direction, notes, and lyrics', () => {
    ...
    const angelusDirection = directionsArray.find(d => d.direction_type?.[0]?.words?.text === 'Angelus dicit:');
    ...
    const barlineElement = measure1.content?.find(item => (item as any)._type === 'barline');
  ...
  it('should parse slurs in Measure 1', () => {
    if (!scorePartwise) throw new Error('scorePartwise is null');
    const measure1 = scorePartwise.parts[0].measures.find(m => m.number === '1');
    if (!measure1) throw new Error('Measure 1 not found');
    const notesInMeasure1 = getNotesFromContent(measure1.content);
    
    const noteWithSlurStart = notesInMeasure1.find(n => n.notations?.slurs?.some(s => s.type === 'start'));
    ...
    const noteWithSlurStop = notesInMeasure1.find(n => n.notations?.slurs?.some(s => s.type === 'stop'));
