...
import { mapBackupElement, mapForwardElement, mapMeasureElement } from '../src/parser/mappers';
...
describe('Backup and Forward mapping', () => {
  ...
  it('maps a <forward> element', () => {
    const xml = '<forward><duration>2</duration><voice>1</voice><staff>2</staff></forward>';
    const el = createElement(xml);
  ...
  it('includes backup and forward in measure content', () => {
    ...
    const notes = measure.content?.filter((c): c is Note => (c as any)._type === 'note' && NoteSchema.safeParse(c).success);
    const backups = measure.content?.filter((c): c is Backup => (c as any)._type === 'backup');
    const forwards = measure.content?.filter((c): c is Forward => (c as any)._type === 'forward');
